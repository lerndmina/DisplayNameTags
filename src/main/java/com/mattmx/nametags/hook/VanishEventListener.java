...
public class VanishEventListener implements Listener {
    ...
    /**
     * When a player vanishes, remove them as a viewer from all nametags they can't see,
    ...
    public void onPlayerShow(@NotNull PlayerShowEvent event) {
        ...
        for (Player viewer : Bukkit.getOnlinePlayers()) {
            if (viewer.equals(shownPlayer) && !showSelf) continue;
            if (!viewer.getWorld().equals(shownPlayer.getWorld())) continue;
